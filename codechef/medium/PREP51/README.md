...
**Runtime:** N/A  
**Memory:** N/A  
**Submitted:** 2026-08-18T05:43:12.804Z  
...
public class Main {
    ...
    private static String minWindowSubstring(String s, String t){
        ...
        // sliding window for the substring each time hitting a needed match
        for (int right = 0; right < s.length(); right++){
            char rChar = s.charAt(right);
            // if this character is needed then include it in window
            if(need[rChar] > 0){
                window[rChar]++;
                // if the count in window is <= need then we've found one required instance
                if(window[rChar] <= need[rChar]){
                    required--;
                }
            }
            
            // when we have a valid window, try and contract it
            while(required == 0){
                ...
                if(need[lChar] > 0){
                    window[lChar]--;
                    // Once window count drops below needed count, increment required
